...
from tscompbench.adapters import Lz4FrameAdapter
...
LIBRARY = PROJECT_ROOT / "build/adapters/lz4_frame/release/libtscb_lz4_frame.so"
...
def _ensure_library() -> None:
    if LIBRARY.is_file():
        return
    subprocess.run(
        [sys.executable, "tools/build_codec.py", "lz4-frame", "--profile", "release"],
        cwd=PROJECT_ROOT,
        check=True,
    )
...
def _adapter_and_manifest():
    ...
    manifest = CodecRegistry(PROJECT_ROOT / "registry/codecs", sources).get("lz4-frame")
    return Lz4FrameAdapter(LIBRARY, manifest.document["adapter"]), manifest


def test_lz4_frame_round_trip_preserves_multiple_buffers_and_ieee_bits() -> None:
    ...
    routed = RoutedInput(
        dataset_id="v2:dataset:lz4-adapter-test",
    ...
    assert observation.encoded.finalize_bytes >= 8
    ...
    assert observation.encoded.ledger.accounting_method == (
        "EXACT_TSCB_CONTAINER_AND_LZ4_FRAME_INSPECTION"
    )
...
def test_lz4_frame_passes_framework_boundary_suite() -> None:
