...
from tscompbench.adapters import BrotliStreamAdapter
...
LIBRARY = PROJECT_ROOT / "build/adapters/brotli_stream/release/libtscb_brotli_stream.so"
...
def _ensure_library() -> None:
    if LIBRARY.is_file():
        return
    subprocess.run(
        [sys.executable, "tools/build_codec.py", "brotli-stream", "--profile", "release"],
        cwd=PROJECT_ROOT,
        check=True,
    )
...
def _adapter_and_manifest():
    ...
    manifest = CodecRegistry(PROJECT_ROOT / "registry/codecs", sources).get("brotli-stream")
    return BrotliStreamAdapter(LIBRARY, manifest.document["adapter"]), manifest


def test_brotli_stream_round_trip_preserves_multiple_buffers_and_ieee_bits() -> None:
    ...
    routed = RoutedInput(
        dataset_id="v2:dataset:brotli-adapter-test",
    ...
    assert observation.encoded.ledger.accounting_method == (
        "EXACT_TSCB_CONTAINER_AND_BROTLI_STREAM_LENGTH"
    )
...
def test_brotli_stream_passes_framework_boundary_suite() -> None:
