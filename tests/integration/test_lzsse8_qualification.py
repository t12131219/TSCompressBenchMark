...
def test_lzsse8_five_layer_qualification_and_append_only_resume(tmp_path):
    config = ROOT / "configs/experiments/lzsse8-raw-qualification.toml"
    run = initialize_run_set(config, tmp_path / "runs", run_set_id="lzsse8-five-layer")
    ...
    resumed = initialize_run_set(
        config, tmp_path / "runs", run_set_id="lzsse8-five-layer", resume=True
    )
    assert resumed.config.profile.cpu_affinity == (0,)
    assert resumed.config.canonical_document == json.loads(
        (run.path / "frozen_config.json").read_text()
    )
