...
def test_lzsse2_five_layer_qualification_and_append_only_resume(tmp_path):
    config = ROOT / "configs/experiments/lzsse2-raw-qualification.toml"
    run = initialize_run_set(config, tmp_path / "runs", run_set_id="lzsse2-five-layer")
    ...
    resumed = initialize_run_set(
        config, tmp_path / "runs", run_set_id="lzsse2-five-layer", resume=True
    )
