    ...
    def __init__(self, path: Path):
        try:
            ...
            raise ExecutionContractError(
                f"cannot load LZ4 adapter artifact {path}: {error}"
            ) from error
        self._bind()
        if int(self.library.tscb_get_abi_version()) != _ABI_VERSION:
            raise ExecutionContractError("LZ4 adapter ABI version mismatch")
        ...
        if status != _STATUS_OK:
            raise ExecutionContractError("LZ4 adapter failed to expose its native manifest")
        manifest = json.loads(ctypes.string_at(manifest_pointer, manifest_length.value))
        if manifest.get("algorithm") != "lz4-frame":
            raise ExecutionContractError("loaded C ABI artifact is not the LZ4 frame adapter")
...
class Lz4FrameAdapter:
    ...
    def create_session(self, parameters: dict[str, Any]) -> Lz4FrameSession:
        return Lz4FrameSession(self.library_path, parameters)


class Lz4FrameSession:
    def __init__(self, library_path: Path, parameters: dict[str, Any]):
        ...
        config = canonical_json_bytes(
            {
                "block_checksum": bool(parameters.get("block_checksum", False)),
                "block_mode": (
                    0
                    if parameters.get("block_mode", "INDEPENDENT") == "INDEPENDENT"
                    else 1
                ),
                "block_size_id": int(parameters.get("block_size_id", 4)),
                "compression_level": int(parameters.get("compression_level", 0)),
        ...
        if status != _STATUS_OK or not self._handle.value:
            raise ExecutionContractError(f"native LZ4 adapter create failed with status {status}")
        self._updated = False
        self._finalized = False
        self._header = b""
        self._native_stream_offset = 0
        try:
    ...
    def _descriptor_header(routed: RoutedInput) -> bytes:
        ...
        return canonical_json_bytes(
            {
                "buffers": descriptors,
                "schema_version": "tscb.lz4-frame-container.v1",
    ...
    def compress_update(self, routed: RoutedInput, destination: memoryview) -> int:
        if self._updated:
            raise ExecutionContractError("compress_update may be called once per LZ4 frame")
        header = self._descriptor_header(routed)
        preamble = _PREFIX.pack(_MAGIC, len(header)) + header
        if len(destination) < len(preamble):
            raise OutputCapacityError("destination cannot hold the LZ4 container header")
        ...
        self._header = header
        self._native_stream_offset = len(preamble)
        self._updated = True
    ...
    def finalize(self, destination: memoryview) -> int:
        ...
        output_buffer = _buffer(
            ctypes.addressof(output_array), capacity=len(destination), used=0
        )
        status = int(
            self._native.library.tscb_finalize(self._handle, ctypes.byref(output_buffer))
        )
    ...
    @staticmethod
    def _parse_container(stream: bytes) -> tuple[dict[str, Any], bytes, int]:
        if len(stream) < _PREFIX.size:
            raise ExecutionContractError("truncated LZ4 container")
        magic, header_length = _PREFIX.unpack_from(stream)
        if magic != _MAGIC:
            raise ExecutionContractError("invalid LZ4 container magic")
        header_end = _PREFIX.size + header_length
        if header_end > len(stream):
            raise ExecutionContractError("truncated LZ4 container metadata")
        try:
            header = json.loads(stream[_PREFIX.size : header_end].decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise ExecutionContractError("invalid LZ4 container metadata") from error
        if header.get("schema_version") != "tscb.lz4-frame-container.v1":
            raise ExecutionContractError("unsupported LZ4 container version")
        return header, stream[header_end:], header_end

    @staticmethod
    def _inspect_frame(frame: bytes) -> tuple[int, int, int]:
        if len(frame) < 11 or int.from_bytes(frame[:4], "little") != _LZ4_FRAME_MAGIC:
            raise ExecutionContractError("invalid LZ4 frame")
        flags = frame[4]
        if flags >> 6 != 1:
            raise ExecutionContractError("unsupported LZ4 frame version")
        block_checksum = bool(flags & 0x10)
        content_size = bool(flags & 0x08)
        content_checksum = bool(flags & 0x04)
        dictionary_id = bool(flags & 0x01)
        cursor = 6 + (8 if content_size else 0) + (4 if dictionary_id else 0) + 1
        if cursor > len(frame):
            raise ExecutionContractError("truncated LZ4 frame header")
        container_bytes = cursor
        payload_bytes = 0
        checksum_bytes = 0
        while True:
            if cursor + 4 > len(frame):
                raise ExecutionContractError("truncated LZ4 block header")
            word = int.from_bytes(frame[cursor : cursor + 4], "little")
            cursor += 4
            container_bytes += 4
            if word == 0:
                break
            block_bytes = word & 0x7FFFFFFF
            if cursor + block_bytes > len(frame):
                raise ExecutionContractError("truncated LZ4 block payload")
            payload_bytes += block_bytes
            cursor += block_bytes
            if block_checksum:
                if cursor + 4 > len(frame):
                    raise ExecutionContractError("truncated LZ4 block checksum")
                checksum_bytes += 4
                cursor += 4
        if content_checksum:
            if cursor + 4 > len(frame):
                raise ExecutionContractError("truncated LZ4 content checksum")
            checksum_bytes += 4
            cursor += 4
        if cursor != len(frame):
            raise ExecutionContractError("trailing bytes after LZ4 frame")
    ...
    def accounting(self, stream: bytes, routed: RoutedInput) -> AccountingLedger:
        ...
        header, frame, _ = self._parse_container(stream)
        if canonical_json_bytes(header) != self._header:
            raise ExecutionContractError("LZ4 descriptor metadata changed after compression")
        ...
        return AccountingLedger.create(
            ...
            accounting_method="EXACT_TSCB_CONTAINER_AND_LZ4_FRAME_INSPECTION",
            **components,
        )
    ...
    def decompress(self, stream: bytes) -> DecodedOutput:
        ...
        if not isinstance(descriptors, list):
            raise ExecutionContractError("LZ4 container has no buffer descriptors")
        ...
        input_buffer = _buffer(
            ctypes.addressof(input_array), capacity=len(frame), used=len(frame)
        )
        output_buffer = _buffer(
            ctypes.addressof(output_array), capacity=output_bytes, used=0
        )
        status = int(
        ...
        if int(output_buffer.used_bytes) != output_bytes:
            raise ExecutionContractError("LZ4 decoded byte count does not match metadata")
        ...
        for descriptor in descriptors:
            ...
            if expected != length or cursor + length > output_bytes:
                raise ExecutionContractError("LZ4 descriptor shape/dtype is inconsistent")
        ...
        if cursor != output_bytes:
            raise ExecutionContractError("LZ4 container contains undeclared decoded bytes")
    ...
    def close(self) -> None:
        if self._handle.value:
            ...
            self._handle = ctypes.c_void_p()
            if status != _STATUS_OK:
                raise ExecutionContractError(f"native LZ4 adapter destroy failed ({status})")
