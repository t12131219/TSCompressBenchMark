    ...
    def __init__(self, path: Path):
        try:
            ...
            raise ExecutionContractError(
                f"cannot load bzip2 stream adapter artifact {path}: {error}"
            ) from error
        self._bind()
        if int(self.library.tscb_get_abi_version()) != _ABI_VERSION:
            raise ExecutionContractError("bzip2 stream adapter ABI version mismatch")
        ...
        if status != _STATUS_OK:
            raise ExecutionContractError(
                "bzip2 stream adapter failed to expose its native manifest"
            )
        manifest = json.loads(ctypes.string_at(manifest_pointer, manifest_length.value))
        if manifest.get("algorithm") != "bzip2-stream":
            raise ExecutionContractError("loaded C ABI artifact is not the bzip2 adapter")
        if (
            manifest.get("pipeline") != "BWT_MTF_RLE_HUFFMAN"
            or manifest.get("stream") != "BZIP2_STREAM"
            or manifest.get("threading") != "SINGLE_THREAD"
        ):
            raise ExecutionContractError(
                "bzip2 artifact violates the registered execution mode"
            )
...
class Bzip2StreamAdapter:
    ...
    def create_session(self, parameters: dict[str, Any]) -> Bzip2StreamSession:
        return Bzip2StreamSession(self.library_path, parameters)


class Bzip2StreamSession:
    def __init__(self, library_path: Path, parameters: dict[str, Any]):
        self._native = _NativeLibrary(library_path)
        self._handle = ctypes.c_void_p()
        self._compression_level = int(parameters.get("compression_level", 9))
        config = canonical_json_bytes({"compression_level": self._compression_level})
        status = int(
        ...
        if status != _STATUS_OK or not self._handle.value:
            raise ExecutionContractError(
                f"native bzip2 stream adapter create failed with status {status}"
            )
        self._updated = False
    ...
    def _descriptor_header(routed: RoutedInput) -> bytes:
        ...
        return canonical_json_bytes(
            {
                "buffers": descriptors,
                "schema_version": "tscb.bzip2-stream-container.v1",
    ...
    def compress_update(self, routed: RoutedInput, destination: memoryview) -> int:
        if self._updated:
            raise ExecutionContractError("compress_update may be called once per bzip2 stream")
        header = self._descriptor_header(routed)
        preamble = _PREFIX.pack(_MAGIC, len(header)) + header
        if len(destination) < len(preamble):
            raise OutputCapacityError("destination cannot hold the bzip2 container header")
    ...
    def _parse_container(stream: bytes) -> tuple[dict[str, Any], bytes]:
        if len(stream) < _PREFIX.size:
            raise ExecutionContractError("truncated bzip2 container")
        magic, header_length = _PREFIX.unpack_from(stream)
        if magic != _MAGIC:
            raise ExecutionContractError("invalid bzip2 container magic")
        header_end = _PREFIX.size + header_length
        if header_end > len(stream):
            raise ExecutionContractError("truncated bzip2 container metadata")
        try:
            header = json.loads(stream[_PREFIX.size : header_end].decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise ExecutionContractError("invalid bzip2 container metadata") from error
        if header.get("schema_version") != "tscb.bzip2-stream-container.v1":
            raise ExecutionContractError("unsupported bzip2 container version")
        return header, stream[header_end:]

    def _inspect_stream(self, stream: bytes) -> None:
        expected = b"BZh" + bytes((ord("0") + self._compression_level,))
        if len(stream) < 14:
            raise ExecutionContractError("truncated bzip2 stream")
        if stream[:4] != expected:
            raise ExecutionContractError(
                "bzip2 stream header does not match the registered compression level"
            )
    ...
    def accounting(self, stream: bytes, routed: RoutedInput) -> AccountingLedger:
        ...
        header, bzip2_stream = self._parse_container(stream)
        if canonical_json_bytes(header) != self._header:
            raise ExecutionContractError("bzip2 descriptor metadata changed after compression")
        self._inspect_stream(bzip2_stream)
        ...
        if routed.track is BenchmarkTrack.TIMESTAMP:
            components["timestamp_bits"] = len(bzip2_stream) * 8
        elif routed.track is BenchmarkTrack.VALUE and not has_validity:
            components["value_bits"] = len(bzip2_stream) * 8
        else:
            components["unallocated_shared_bits"] = len(bzip2_stream) * 8
        return AccountingLedger.create(
            track=routed.track,
            canonical_raw_bits=routed.canonical_raw_bits,
            final_physical_bytes=len(stream),
            accounting_method="EXACT_TSCB_CONTAINER_PLUS_COMPLETE_BZIP2_STREAM_LENGTH",
            **components,
        )

    def decompress(self, stream: bytes) -> DecodedOutput:
        header, frame = self._parse_container(stream)
        self._inspect_stream(frame)
        descriptors = header.get("buffers")
        if not isinstance(descriptors, list):
            raise ExecutionContractError("bzip2 container has no buffer descriptors")
        try:
            output_bytes = sum(int(item["payload_bytes"]) for item in descriptors)
        except (KeyError, TypeError, ValueError) as error:
            raise ExecutionContractError("invalid bzip2 buffer descriptor") from error
        ...
        if int(output_buffer.used_bytes) != output_bytes:
            raise ExecutionContractError("bzip2 decoded byte count does not match metadata")
        ...
        try:
            for descriptor in descriptors:
                ...
                if expected != length or cursor + length > output_bytes:
                    raise ExecutionContractError(
                        "bzip2 descriptor shape/dtype is inconsistent"
                    )
        ...
        if cursor != output_bytes:
            raise ExecutionContractError("bzip2 container contains undeclared decoded bytes")
    ...
    def close(self) -> None:
        if self._handle.value:
            ...
            if status != _STATUS_OK:
                raise ExecutionContractError(
                    f"native bzip2 stream adapter destroy failed ({status})"
                )
