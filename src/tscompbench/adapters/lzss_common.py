...
class _NativeLibrary:
    def __init__(self, path: Path, variant: _LzssVariant):
        try:
        ...
        if manifest.get("algorithm") != variant.algorithm:
            raise ExecutionContractError("loaded C ABI artifact is not the LZSS adapter")
        if (
            manifest.get("dictionary") != variant.dictionary
            or manifest.get("stream") != variant.stream
...
class _LzssSession:
    _VARIANT: _LzssVariant

    def __init__(self, library_path: Path, parameters: dict[str, Any]):
        self._native = _NativeLibrary(library_path, self._VARIANT)
        self._handle = ctypes.c_void_p()
        config = canonical_json_bytes(
            {
                key: parameters.get(key, value)
                for key, value in self._VARIANT.native_config.items()
            }
        )
        status = int(
    ...
    def _descriptor_header(self, routed: RoutedInput) -> bytes:
        descriptors = [
            {
        ...
        return canonical_json_bytes(
            {
                "buffers": descriptors,
                "schema_version": self._VARIANT.schema_version,
    ...
    def compress_update(self, routed: RoutedInput, destination: memoryview) -> int:
        ...
        header = self._descriptor_header(routed)
        preamble = _PREFIX.pack(self._VARIANT.magic, len(header)) + header
    ...
    @classmethod
    def _parse_container(cls, stream: bytes) -> tuple[dict[str, Any], bytes]:
        ...
        if header.get("schema_version") != cls._VARIANT.schema_version:
            raise ExecutionContractError("unsupported LZSS container version")
        return header, stream[header_end:]

    @classmethod
    def _inspect_lzss_stream(cls, stream: bytes, expected_bytes: int) -> dict[str, int]:
        ...
        while produced < expected_bytes:
            literal = read(1)
            token = read(
                8 if literal else cls._VARIANT.offset_bits + cls._VARIANT.length_bits
            )
            length_mask = (1 << cls._VARIANT.length_bits) - 1
            produced += 1 if literal else (token & length_mask) + cls._VARIANT.minimum_match
    ...
    def accounting(self, stream: bytes, routed: RoutedInput) -> AccountingLedger:
        ...
        header, raw_stream = self._parse_container(stream)
        ...
        parts = self._inspect_lzss_stream(
            raw_stream, sum(item.array.nbytes for item in routed.buffers)
        )
        ...
        return AccountingLedger.create(
            ...
            accounting_method=self._VARIANT.accounting_method,
            **components,
        )
