    ...
    def __init__(self, path: Path):
        try:
            ...
            raise ExecutionContractError(
                f"cannot load LZSSE8 adapter artifact {path}: {error}"
            ) from error
        self._bind()
        if int(self.library.tscb_get_abi_version()) != _ABI_VERSION:
            raise ExecutionContractError("LZSSE8 adapter ABI version mismatch")
        ...
        if status != _STATUS_OK:
            raise ExecutionContractError("LZSSE8 adapter failed to expose its native manifest")
        manifest = json.loads(ctypes.string_at(manifest_pointer, manifest_length.value))
        if manifest.get("algorithm") != "lzsse8-raw":
            raise ExecutionContractError("loaded C ABI artifact is not the LZSSE8 raw adapter")
        if (
            ...
            or manifest.get("isa") != "SSE4_1"
        ):
            raise ExecutionContractError("LZSSE8 artifact violates the registered execution mode")
...
class Lzsse8RawAdapter:
    ...
    def create_session(self, parameters: dict[str, Any]) -> Lzsse8RawSession:
        return Lzsse8RawSession(self.library_path, parameters)


class Lzsse8RawSession:
    def __init__(self, library_path: Path, parameters: dict[str, Any]):
        if (
            type(parameters.get("compression_level", 12)) is not int
            or parameters.get("compression_level", 12) != 12
            or parameters.get("content_checksum", False) is not False
            or type(parameters.get("native_timing", True)) is not bool
            or parameters.get("isa", "SSE4_1") != "SSE4_1"
        ):
            raise ExecutionContractError("LZSSE8 variant or timing parameters are not registered")
        self._native = _NativeLibrary(library_path)
        self._handle = ctypes.c_void_p()
        config = canonical_json_bytes(
            {
                "compression_level": int(parameters.get("compression_level", 12)),
        ...
        if status != _STATUS_OK or not self._handle.value:
            raise ExecutionContractError(
                f"native LZSSE8 adapter create failed with status {status}"
            )
        self._updated = False
        ...
        return canonical_json_bytes(
            {
                "buffers": descriptors,
                "raw_storage": 0,
                "schema_version": "tscb.lzsse8-raw-container.v1",
    ...
    def compress_update(self, routed: RoutedInput, destination: memoryview) -> int:
        if self._updated:
            raise ExecutionContractError("compress_update may be called once per LZSSE8 raw stream")
        header = self._descriptor_header(routed)
        preamble = _PREFIX.pack(_MAGIC, len(header)) + header
        if len(destination) < len(preamble):
            raise OutputCapacityError("destination cannot hold the LZSSE8 container header")
        ...
        metadata["raw_storage"] = int(output_buffer.used_bytes == len(payload))
    ...
    def finalize(self, destination: memoryview) -> int:
        ...
        output_buffer = _buffer(1, capacity=0, used=0)
    ...
    def _parse_container(stream: bytes) -> tuple[dict[str, Any], bytes]:
        if len(stream) < _PREFIX.size:
            raise ExecutionContractError("truncated LZSSE8 container")
        magic, header_length = _PREFIX.unpack_from(stream)
        if magic != _MAGIC:
            raise ExecutionContractError("invalid LZSSE8 container magic")
        header_end = _PREFIX.size + header_length
        if header_length > 1048576 or header_end > len(stream):
            raise ExecutionContractError("truncated LZSSE8 container metadata")
        try:
            header = json.loads(stream[_PREFIX.size : header_end].decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise ExecutionContractError("invalid LZSSE8 container metadata") from error
        if not isinstance(header, dict):
            raise ExecutionContractError("invalid LZSSE8 metadata object")
        if header.get("schema_version") != "tscb.lzsse8-raw-container.v1":
            raise ExecutionContractError("unsupported LZSSE8 container version")
    ...
    def accounting(self, stream: bytes, routed: RoutedInput) -> AccountingLedger:
        ...
        if canonical_json_bytes(header) != self._header:
            raise ExecutionContractError("LZSSE8 descriptor metadata changed after compression")
        ...
        return AccountingLedger.create(
            ...
            accounting_method="EXACT_TSCB_CONTAINER_AND_LZSSE8_RAW_LENGTH",
            **components,
        )
    ...
    def decompress(self, stream: bytes) -> DecodedOutput:
        ...
        if not isinstance(descriptors, list):
            raise ExecutionContractError("LZSSE8 container has no buffer descriptors")
        output_bytes = 0
        names = set()
        try:
            for item in descriptors:
                ...
                for dimension in shape:
                    ...
                    elements *= dimension
                if type(length) is not int or length != elements * dtype.itemsize:
                    raise ValueError("invalid payload length")
                if type(item["logical_bits"]) is not int or item["logical_bits"] != length * 8:
                    raise ValueError("invalid logical bits")
                if not isinstance(item["name"], str) or item["name"] in names:
                    raise ValueError("invalid or duplicate name")
                names.add(item["name"])
                output_bytes += length
                if output_bytes > 2147483647:
                    raise ValueError("decoded length exceeds registered limit")
        except (KeyError, TypeError, ValueError) as error:
            raise ExecutionContractError("invalid LZSSE8 descriptors") from error
        if type(header.get("raw_storage")) is not int or header["raw_storage"] not in (0, 1):
            raise ExecutionContractError("invalid LZSSE8 raw storage flag")
        if bool(header["raw_storage"]) != (len(frame) == output_bytes):
            raise ExecutionContractError("LZSSE8 raw storage flag disagrees with stream length")
        ...
        if int(output_buffer.used_bytes) != output_bytes:
            raise ExecutionContractError("LZSSE8 decoded byte count does not match metadata")
        ...
        for descriptor in descriptors:
            ...
            if expected != length or cursor + length > output_bytes:
                raise ExecutionContractError("LZSSE8 descriptor shape/dtype is inconsistent")
        ...
        if cursor != output_bytes:
            raise ExecutionContractError("LZSSE8 container contains undeclared decoded bytes")
    ...
    def close(self) -> None:
        if self._handle.value:
            ...
            self._handle = ctypes.c_void_p()
            if status != _STATUS_OK:
                raise ExecutionContractError(f"native LZSSE8 adapter destroy failed ({status})")
