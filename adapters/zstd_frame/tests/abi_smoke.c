...
static int round_trip(size_t size) {
    static const char config[] =
        "{\"compression_level\":3,\"content_checksum\":true}";
    ...
    final_output = buffer(compressed + output.used_bytes, bound - output.used_bytes, 0);
...
int main(void) {
    ...
    if (round_trip(0) || round_trip(1) || round_trip(2) || round_trip(131071)
        || round_trip(131072) || round_trip(131073) || round_trip(262145)) {
        fputs("Zstd C ABI smoke failed\n", stderr);
        return 1;
    }
    puts("Zstd C ABI smoke passed");
    return 0;
}
