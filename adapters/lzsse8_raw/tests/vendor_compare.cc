...
int main() {
    ...
    for(size_t n: {1,2,31,32,33,64,257,1024,4097,65536,131073}) {
        for(unsigned kind=0;kind<4;++kind) {
            ...
            auto* state=LZSSE8_MakeOptimalParseState(n);
            assert(state);
            size_t size=LZSSE8_CompressOptimalParse(state,input.data(),n,original.data(),n,12);
            ...
            assert(LZSSE8_Decompress(adapted.data(),size,output.data(),n)==n);
            assert(input==output);
