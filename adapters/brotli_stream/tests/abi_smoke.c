...
static int round_trip(size_t size) {
    static const char config[] = "{\"compression_level\":5,\"window_bits\":22}";
...
int main(void) {
    if (tscb_get_abi_version() != TSCB_ADAPTER_ABI_V1 || round_trip(0U)
        ...
        fputs("Brotli stream C ABI smoke failed\n", stderr);
        return 1;
    }
    puts("Brotli stream C ABI smoke passed");
    return 0;
}
