...
int main() {
    ...
    unsigned cases = 0;
    for (size_t n: {0,1,2,3,4,5,7,8,9,15,16,17,31,32,33,63,64,65,255,256,257,
        ...
        for (unsigned pattern = 0; pattern < 5; ++pattern) {
            ...
            if (n) {
                auto alias = buffer(input.data,n,0);
                assert(tscb_compress(handle,&in,&alias) == TSCB_STATUS_INVALID_ARGUMENT_V1);
            }
            ...
            if (out.used_bytes) {
                auto alias = buffer(exact.data,out.used_bytes,0);
                assert(tscb_decompress(handle,&stream,&alias) == TSCB_STATUS_INVALID_ARGUMENT_V1);
            }
