...
static const char TSCB_MANIFEST_JSON[] =
    "{\"abi_version\":1,\"algorithm\":\"lzsse2-raw\","
    "\"checksum\":\"NONE\",\"framing\":\"NONE\","
    "\"dictionary\":\"NONE\",\"isa\":\"SSE4_1\","
    "\"library\":\"lzsse2-2019-04-18-from-lzbench\","
    "\"stream\":\"LZSSE2_RAW\",\"threading\":\"SINGLE_THREAD\"}";
...
static bool valid_stream(const uint8_t* data, size_t size, size_t decoded) {
    ...
    if (size >= decoded || size < 35 || decoded < 32) return false;
    size_t in = 3, out = 3, offset = 3;
    bool carry = false;
    while (out < decoded - 16 && in < size - 16) {
        ...
        for (unsigned k = 0; k < 32; ++k) {
            ...
            size_t count = carry ? c : c + 1;
            size_t read = carry ? 0 : (c < 2 ? count : 2);
            if (size - in < 2 || decoded - out < 16) return false;
            if (!carry && c >= 2) {
                offset ^= static_cast<size_t>(data[in]) | (static_cast<size_t>(data[in + 1]) << 8);
            }
            if (offset < 3 || offset > out || count > decoded - 16 - out
                || read > size - 16 - in) return false;
            // Short overlapping vector matches do not reproduce scalar LZ semantics.
            if ((carry || c >= 2) && offset < 16 && count > offset) return false;
            out += count;
            in += read;
            carry = c == 15;
...
tscb_status_v1 tscb_compress_bound(
    ...
    if (input->used_bytes > static_cast<uint64_t>(std::numeric_limits<int32_t>::max())) {
        set_error(handle, "LZSSE2 input exceeds its signed 32-bit position limit");
...
tscb_status_v1 tscb_compress(
    ...
    if (handle->update_called || handle->finalized) {
        set_error(handle, "compress update may be called exactly once per LZSSE2 raw stream");
        return TSCB_STATUS_CODEC_ERROR_V1;
    }
    ...
    if (output->capacity_bytes < bound) {
        set_error(handle, "LZSSE2 raw compression requires input-length capacity");
    ...
    size_t written = 0;
    {
        LZSSE2_OptimalParseState* state = LZSSE2_MakeOptimalParseState(
            static_cast<size_t>(input->used_bytes == 0 ? 1 : input->used_bytes));
        if (state == nullptr) {
            set_error(handle, "LZSSE2 optimal parse state allocation failed");
            return TSCB_STATUS_CODEC_ERROR_V1;
        }
        TSCB_TIME_CODEC(handle->native_timer, encode_ns,
            written = LZSSE2_CompressOptimalParse(
                state,
        ...
        if ((written == 0 && input->used_bytes != 0) || written > input->used_bytes) {
            set_error(handle, "LZSSE2 compression failed or exceeded its raw-stream bound");
            return TSCB_STATUS_CODEC_ERROR_V1;
        }
    }
...
tscb_status_v1 tscb_finalize(tscb_codec_handle_v1* handle, tscb_buffer_v1* output) {
    ...
    std::snprintf(
        ...
        ",\"lzsse2_raw_stream_bytes\":%" PRIu64 "}",
...
tscb_status_v1 tscb_decompress(
    ...
    if (output->capacity_bytes > static_cast<uint64_t>(std::numeric_limits<int32_t>::max())) {
        set_error(handle, "LZSSE2 decoded length exceeds its signed 32-bit limit");
        return TSCB_STATUS_UNSUPPORTED_V1;
    }
    if (overlaps(input, output)) return TSCB_STATUS_INVALID_ARGUMENT_V1;
...
tscb_status_v1 tscb_query(
    ...
    set_error(handle, "LZSSE2 raw adapter does not provide random access queries");
    return TSCB_STATUS_UNSUPPORTED_V1;
}
