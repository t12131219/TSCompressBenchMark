...
static int round_trip(size_t size) {
    static const char config[] = "{\"compression_level\":6}";
        ...
        || tscb_create(config, sizeof(config) - 1U, &decode) != TSCB_STATUS_OK_V1) goto cleanup;
    {
        ...
        compressed[compressed_input.used_bytes - 1U] ^= 1U;
        {
            uint64_t offsets[4] = {8U, compressed_input.used_bytes - 16U,
                                  compressed_input.used_bytes - 12U, 0U};
            size_t count = 3U;
            if (size != 0U) {
                offsets[count++] = 12U + ((uint64_t)compressed[12U] + 1U) * 4U - 4U;
            }
            for (index = 0U; index < count; ++index) {
                compressed[offsets[index]] ^= 1U;
                if (tscb_decompress(decode, &compressed_input, &decoded_output)
                    == TSCB_STATUS_OK_V1) goto cleanup;
                compressed[offsets[index]] ^= 1U;
            }
        }
        if (compressed_input.used_bytes < bound) {
            compressed[compressed_input.used_bytes] = 0U;
            compressed_input.used_bytes += 1U;
            compressed_input.capacity_bytes += 1U;
            if (tscb_decompress(decode, &compressed_input, &decoded_output)
                == TSCB_STATUS_OK_V1) goto cleanup;
            compressed_input.used_bytes -= 1U;
            compressed_input.capacity_bytes -= 1U;
        ...
        if (tscb_reset(encode, 0U) != TSCB_STATUS_OK_V1
            ...
            || timing.native_decode_wall_ns != 0U) goto cleanup;
        output = buffer(compressed, 1U, 0U);
        if (tscb_compress(encode, &input, &output) != TSCB_STATUS_DST_TOO_SMALL_V1) goto cleanup;
...
int main(void) {
    if (tscb_get_abi_version() != TSCB_ADAPTER_ABI_V1 || round_trip(0U)
        ...
        fputs("XZ LZMA2 C ABI smoke failed\n", stderr);
        return 1;
    }
    puts("XZ LZMA2 C ABI smoke passed");
    return 0;
}
