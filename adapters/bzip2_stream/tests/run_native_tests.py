...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="tscb-bzip2-native-") as temporary:
        for profile in ("release", "sanitizer"):
            library_directory = PROJECT_ROOT / f"build/adapters/bzip2_stream/{profile}"
            if not (library_directory / "libtscb_bzip2_stream.so").is_file():
                raise FileNotFoundError(f"build {profile} before running native tests")
            for name, include in (
                ("abi_smoke", PROJECT_ROOT / "native/include"),
                ("vendor_roundtrip", ADAPTER / "vendor/bzip2"),
            ):
                ...
                if profile == "release" and name == "abi_smoke":
                    competitor = Path(temporary) / "competing-bzip2.so"
                    subprocess.run(
                        [
                            os.environ.get("CC", "cc"),
                            "-shared",
                    ...
                    print("release: competing global libbz2 symbol rejection", flush=True)
                    subprocess.run(
