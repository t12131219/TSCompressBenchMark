"""Guard/canary and sanitizer qualification for the retained LZSSE8 source closure."""
...
def main():
    ...
    for profile in ("release", "sanitizer"):
        directory = ROOT / f"build/adapters/lzsse8_raw/{profile}"
        ...
        command = [
            ...
            *(
            ...
            str(ROOT / "adapters/lzsse8_raw/tests/qualification.cc"),
            "-L",
            str(directory),
            "-ltscb_lzsse8_raw",
        ...
        for invocation in (command, [str(executable)]):
            ...
            (ROOT / "build/source-audits/lzsse8-native-tests-20260918.log").write_text(
                "\n".join(log)
            )
            print(result.stdout, flush=True)
            if result.returncode:
                raise RuntimeError(f"LZSSE8 {profile} qualification failed: {result.returncode}")
        records.append(
            {
                "profile": profile,
                "suite": "project-exact-buffer-guard-canary-180-roundtrips-2048-hostile-streams",
                "status": "PASS",
            }
        )
    vendor = ROOT / "adapters/lzsse8_raw/vendor/lzsse/lzsse8"
    directory = ROOT / "build/adapters/lzsse8_raw/release"
    command = [
        "c++",
        "-std=c++17",
        "-O2",
        ...
        "-I",
        str(vendor),
        str(ROOT / "adapters/lzsse8_raw/tests/vendor_compare.cc"),
        str(vendor / "lzsse8.cpp"),
        "-L",
        str(directory),
        "-ltscb_lzsse8_raw",
        f"-Wl,-rpath,{directory}",
    ...
    for profile in ("release", "sanitizer"):
        ...
        command = [
            ...
            *(["-fsanitize=address,undefined", "-no-pie"] if profile == "sanitizer" else []),
            ...
            "-I",
            str(vendor),
            str(ROOT / "adapters/lzsse8_raw/tests/allocation_failure.cc"),
            "-o",
            str(executable),
        ]
    ...
    log_path = ROOT / "build/source-audits/lzsse8-native-tests-20260918.log"
    ...
    (ROOT / "build/source-audits/lzsse8-native-tests-20260918.json").write_text(
