"""Audit authoritative LZSSE8 five-layer raw/report/source evidence."""
...
def main():
    ...
    card = validate_onboarding_card(
        json.loads((ROOT / "registry/onboarding/lzsse8-raw.json").read_text())
    )
    registry = CodecRegistry(ROOT / "registry/codecs", SourceRegistry(ROOT / "registry/sources"))
    assert card["source_artifact_id"] == registry.get("lzsse8-raw").source_artifact_id
    for build in card["builds"]:
        directory = ROOT / f"build/adapters/lzsse8_raw/{build['profile']}"
        binary = directory / "libtscb_lzsse8_raw.so"
        ...
        assert (
            hashlib.sha256((directory / "lzsse8.cpp").read_bytes()).hexdigest()
            == command["patched_translation_unit_sha256"]
        )
    vendor = ROOT / "adapters/lzsse8_raw/vendor/lzsse"
    ...
    header, payload = Lzsse8RawSession._parse_container(stream_files[0].read_bytes())
    ...
    output = ROOT / "build/source-audits/lzsse8-final-audit-20260918.json"
